...
const loadNotifications = async (reset = false) => {
  ...
  try {
    ...
    // 检测到新通知时，显示系统通知
    // 如果：新未读数 > 旧未读数，才显示新通知
    if (newUnreadCount > previousUnreadCount || (previousUnreadCount === 0 && newUnreadCount > 0)) {
    ...
    if (!visible.value) {
      ...
      getNotifications({ page: 1, page_size: 1 }).then(res => {
        ...
        if (newUnreadCount > previousUnreadCount && previousUnreadCount > 0) {
          console.log('检测到新通知:', newUnreadCount - previousUnreadCount)
          // 获取最新的未读通知
