<template>
  <div class="article-editor-page">
    ...
    <CoverMakerDialog v-model="coverMakerVisible" :title="formData.title" :author="authorInfo.name"
      :avatar="authorInfo.avatar" @save="handleCoverMakerSave" />
  </div>
</template>
...
const saveDraftSilently = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      ...
      await articleStore.updateArticleById(id, saveData)
    } else if (draftArticleId.value) {
      // 新建模式：更新已创建的草稿（保持未发布状态）
      saveData.is_publish = false
      await articleStore.updateArticleById(draftArticleId.value, saveData)
    } else {
      // 新建模式：首次保存，创建新草稿
      saveData.is_publish = false
      saveData.publish_time = ''
      saveData.update_time = ''
      const result = await articleStore.addArticle(saveData)
...
const handleSave = async (autoRedirect: boolean = true) => {
  ...
  try {
    ...
    if (isEdit.value) {
      await articleStore.updateArticleById(id, submitData)
      ElMessage.success('更新文章成功')
    } else if (draftArticleId.value) {
      // 新建模式：如果已有自动保存的草稿，更新它而不是创建新文章
      await articleStore.updateArticleById(draftArticleId.value, submitData)
      ElMessage.success(submitData.is_publish ? '文章已发布' : '保存成功')
    } else {
      await articleStore.addArticle(submitData)
...
const handleCategorySelect = async (value: any) => {
  ...
  // 创建新分类
  const newCategory = await createNewItem(categoryName, articleStore.addCategory, articleStore.fetchCategories, 'category')
  if (newCategory) {
...
const handleTagSelect = async (values: any[]) => {
  ...
  if (newNames.length > 0) {
    ...
    for (const tagName of newNames) {
      const newTag = await createNewItem(tagName, articleStore.addTag, articleStore.fetchTags, 'tag')
      if (newTag) {
...
const initData = async () => {
  await Promise.all([articleStore.fetchCategories(), articleStore.fetchTags(), fetchSiteSettings()])
