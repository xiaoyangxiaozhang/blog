...
import { formatForBackend, formatTime } from '@/utils/date'
...
watch(() => momentStore.currentMoment, (moment) => {
  ...
  if (moment) {
    ...
    if (moment.content.images?.length) {
      imageItems.value = moment.content.images.map((url: any) => ({
        id: `${Date.now()}-${Math.random()}`,
        type: 'url' as const,
        url: url
      }))
...
const handleCancel = () => {
  momentStore.setDialogVisible(false)
  resetForm()
}
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (momentStore.currentMoment&&momentStore.dialogTitle==='编辑动态') {
      await momentStore.updateMoment(momentStore.currentMoment.id, saveData as UpdateMomentRequest)
      ElMessage.success('更新成功')
    } else {
      await momentStore.createMoment(saveData as CreateMomentRequest)
